...
const handleGuildCreate = (client, guild) => {
  ...
  if (numOfGuilds > 1) {
    logger.warn(
      `Joined the guild ${guild.name}. I'm designed to work only on one server. Currently I'm connected to ${numOfGuilds} servers, unexpected behavior may occur.`
    );
  } else {
...
const handleGuildDelete = (client, guild) => {
  ...
  if (numOfGuilds > 1) {
    logger.warn(
      `Left the guild ${guild.name}. I'm designed to work only on one server. Currently I'm connected to ${numOfGuilds} servers, unexpected behavior may occur.`
    );
  } else if (numOfGuilds === 0) {
    logger.warn(
      `Left the guild ${guild.name}! I'm not connected to any server! :( Please invite me to a server and restart me.`
    );
  } else {
...
const handleInvalidated = () => {
  logger.fatal(
    'Client connection invalidated, terminating execution with code 1.'
  );
  process.exit(1);
};
...
const handleVoiceStateUpdate = (player, oldState, newState) => {
  ...
  if (
    botWasInOldChannel &&
    !botIsInNewChannel &&
    newState.id === player.client.user.id
  ) {
  ...
  if ((!oldChannel && botIsInNewChannel) || botIsInNewChannel) {
    logger.info(
      `User ${newState.member.displayName} has joined ${player.channel.name}.`
    );
    player.updateListeners();
    player.updateDispatcherStatus();
    return;
  ...
  if ((!newChannel && botWasInOldChannel) || botWasInOldChannel) {
    logger.info(
      `User ${oldState.member.displayName} has left ${player.channel.name}.`
    );
    player.updateListeners();
    player.updateDispatcherStatus();
  }
